...
public class WordCountDSL {
    ...
    public static void main(String[] args) throws InterruptedException {
        ...
        // stream.flatMapValues(values ->
        // Arrays.asList(values.toLowerCase().split(" ")))
        // .map((k, v) -> KeyValue.<String, String>pair(v,
        // v)).groupByKey().aggregate(
        // () -> 0L,
        ...
        KStream<String, String> kStream =
                stream
                    .flatMapValues(values -> Arrays.asList(values.toLowerCase().split(" ")))
                    .map((k, v) -> KeyValue.<String, String> pair(v, v))
                    .groupByKey()
                    .aggregate(() -> 0L, (aggKey, value, aggregate) -> aggregate + 1L,
                        TimeWindows.of(5000).advanceBy(5000), Serdes.Long(), "Counts")
                    .toStream()
                    .map(
                        (Windowed<String> window, Long value) -> {
                            return new KeyValue<String, String>(window.key(), String.format(
                                "key=%s, value=%s, start=%d, end=%d\n", window.key(), value, window.window()
                                    .start(), window.window().end()));
                        });
        kStream.to(Serdes.String(), Serdes.String(), "count");

        // KTable<String, Long> kTable = stream.flatMapValues(values ->
        // Arrays.asList(values.toLowerCase().split(" ")))
        // .map((k, v) -> KeyValue.<String, String>pair(v,
        // v)).groupByKey().count("Counts");
